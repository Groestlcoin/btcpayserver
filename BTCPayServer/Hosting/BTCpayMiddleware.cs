...
namespace BTCPayServer.Hosting
{
    public class BTCPayMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            {
                Logs.PayServer.LogCritical(new EventId(), ex, "Unhandled exception in GRSPayMiddleware");
                throw;
            }
        }
            ...
            if (_Options.ExternalUrl != null)
            {
                if (reverseProxyScheme != null && _Options.ExternalUrl.Scheme != reverseProxyScheme)
                {
                    if (reverseProxyScheme == "http" && _Options.ExternalUrl.Scheme == "https")
                        Logs.PayServer.LogWarning($"GRSPay ExternalUrl setting expected to use scheme '{_Options.ExternalUrl.Scheme}' externally, but the reverse proxy uses scheme '{reverseProxyScheme}' (X-Forwarded-Port), forcing ExternalUrl");
                }
