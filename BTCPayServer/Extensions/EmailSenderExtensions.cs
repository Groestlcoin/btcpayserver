...
namespace BTCPayServer.Services
{
    public static class EmailSenderExtensions
    {
        ...
        private static string HEADER_HTML = "<h1 style='font-size:1.2rem'>GRSPay Server</h1><br/>";
        ...
        {
            emailSender.SendEmail(address, "GRSPay Server: Confirm your email",
        ...
        {
            emailSender.SendEmail(address, "GRSPay Server: Your account has been approved",
        ...
        {
            var body = $"A request has been made to reset your GRSPay Server password. Please set your password by clicking below.<br/><br/>{CallToAction("Update Password", HtmlEncoder.Default.Encode(link))}";
            emailSender.SendEmail(address, "GRSPay Server: Update Password", $"<html><body style='{BODY_STYLE}'>{HEADER_HTML}{body}</body></html>");
        }

        public static void SendInvitation(this IEmailSender emailSender, MailboxAddress address, string link)
        {
            emailSender.SendEmail(address, "GRSPay Server: Invitation",
        ...
        {
            emailSender.SendEmail(address, $"GRSPay Server: {newUserInfo}",
