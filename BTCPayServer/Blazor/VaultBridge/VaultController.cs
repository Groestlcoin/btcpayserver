...
public abstract class VaultController : IController
{
    ...
    public async Task Run(VaultBridgeUI ui, CancellationToken cancellationToken)
    {
        try
        {
            ui.ShowFeedback(FeedbackType.Loading, ui.StringLocalizer["Checking GRSPay Server Vault is running..."]);
            ...
            var feedback = (status: res.HttpCode, browser: res.Browser) switch
            {
                ...
                (_, "safari") => new Feedback(
                    ui.ViewLocalizer[
                        "Safari doesn't support GRSPay Server Vault. Please use a different browser. (<a class=\"alert-link\" href=\"https://bugs.webkit.org/show_bug.cgi?id=171934\" target=\"_blank\" rel=\"noreferrer noopener\">More information</a>)"],
                    FeedbackType.Failed),
                _ => new Feedback(
                    ui.ViewLocalizer[
                        "GRSPay Server Vault does not seem to be running, you can download it on <a target=\"_blank\" href=\"https://github.com/groestlcoin/GRSPayServer.Vault/releases/latest\">Github</a>."],
                    FeedbackType.Failed),
            };
            ...
            if (res.HttpCode != 200)
            {
                if (res.HttpCode == 0 && res.Browser == "brave")
                    ui.AddWarning(ui.ViewLocalizer[
                        "Brave supports GRSPay Server Vault, but you need to disable Brave Shields. (<a class=\"alert-link\" href=\"https://www.updateland.com/how-to-turn-off-brave-shields/\" target=\"_blank\" rel=\"noreferrer noopener\">More information</a>)"]);
                ui.ShowRetry();
                return;
            }
