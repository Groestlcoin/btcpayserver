...
function inputChanges(event, buttonSize) {
    ...
    html += srvModel.payButtonText
        ? `<button type="submit" class="submit" name="submit" style="min-width:${width};min-height:${height};border-radius:4px;border-style:none;background-color:#0f3b21;cursor:pointer;" title="Pay with GRSPay Server, a Self-Hosted Groestlcoin Payment Processor"><span style="color:#fff">${esc(srvModel.payButtonText)}</span>\n` +
          ...
          '</button>'
        : `  <input type="image" class="submit" name="submit" src="${esc(srvModel.payButtonImageUrl)}" style="width:${width}" alt="Pay with GRSPay Server, a Self-Hosted Groestlcoin Payment Processor">\n`;
    html += '</form>';
...
function addSelectCurrency(currency) {
    ...
    const defaultCurrencies = ['USD', 'GBP', 'EUR', 'GRS'];
