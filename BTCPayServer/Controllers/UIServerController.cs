...
namespace BTCPayServer.Controllers
{
    ...
    public partial class UIServerController : Controller
    {
        ...
        public IActionResult Maintenance()
        {
            ...
            if (!vm.CanUseSSH)
                TempData[WellKnownTempData.ErrorMessage] = StringLocalizer["Maintenance feature requires access to SSH properly configured in GRSPay Server configuration."].Value;
        ...
        public async Task<IActionResult> Maintenance(MaintenanceViewModel vm, string command)
        {
            ...
            if (command == "changedomain")
            {
                ...
                try
                {
                    ...
                    if (!hasCommonAddress)
                    {
                        ModelState.AddModelError(nameof(vm.DNSDomain), $"Invalid host ({vm.DNSDomain} is not pointing to this GRSPay instance)");
                        return View(vm);
                    }
                }
                catch (Exception ex)
            ...
            else if (command == "restart")
            {
                ...
                TempData[WellKnownTempData.SuccessMessage] = StringLocalizer["GRSPay will restart momentarily."].Value;
            }
        ...
        public async Task<IActionResult> DeleteDynamicDnsService(string hostname)
        {
            ...
            return View("Confirm",
                new ConfirmModel("Delete dynamic DNS service",
                    $"Deleting the dynamic DNS service for <strong>{Html.Encode(hostname)}</strong> means your GRSPay Server will stop updating the associated DNS record periodically.", StringLocalizer["Delete"]));
        }
        ...
        {
            return View("Confirm", new ConfirmModel(StringLocalizer["Disable modification of SSH settings"], StringLocalizer["This action is permanent and will remove the ability to change the SSH settings via the GRSPay Server user interface."], StringLocalizer["Disable"]));
        }
        ...
        public async Task<IActionResult> SSHServiceDisablePost()
        {
            ...
            TempData[WellKnownTempData.SuccessMessage] = StringLocalizer["Changes to the SSH settings are now permanently disabled in the GRSPay Server user interface"].Value;
            return RedirectToAction(nameof(Services));
        }
        ...
        public async Task<IActionResult> Emails(ServerEmailsViewModel model, string command)
        {
            if (command == "Test")
            {
                try
                {
                    ...
                    var serverName = string.IsNullOrEmpty(serverSettings?.ServerName) ? "GRSPay Server" : serverSettings.ServerName;
                    using (var client = await model.Settings.CreateSmtpClient())
                    using (var message = model.Settings.CreateMailMessage(MailboxAddress.Parse(model.TestEmail), $"{serverName}: Email test", "You received it, the GRSPay Server SMTP settings work.", false))
                    {
