        ...
        public async Task<IActionResult> DeleteDynamicDnsService(string hostname)
        {
            ...
            return View("Confirm",
                new ConfirmModel("Delete dynamic DNS service",
                    $"Deleting the dynamic DNS service for <strong>{hostname}</strong> means your GRSPay Server will stop updating the associated DNS record periodically.", "Delete"));
        }
        ...
        public async Task<IActionResult> Emails(EmailsViewModel model, string command)
        {
            ...
            if (command == "Test")
            {
                try
                {
                    ...
                    using (var message = model.Settings.CreateMailMessage(new MailboxAddress(model.TestEmail, model.TestEmail), "GRSPay test", "GRSPay test", false))
                    {
