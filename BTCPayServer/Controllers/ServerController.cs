    ...
    public partial class ServerController : Controller
    {
        ...
        private IActionResult RunSSH(MaintenanceViewModel vm, string ssh)
        {
            ...
            if (_Options.TrustedFingerprints.Count != 0)
            {
                sshClient.HostKeyReceived += (object sender, Renci.SshNet.Common.HostKeyEventArgs e) =>
                {
                    if (_Options.TrustedFingerprints.Count == 0)
                    {
                        Logs.Configuration.LogWarning($"SSH host fingerprint for {e.HostKeyName} is untrusted, start GRSPay with -sshtrustedfingerprints \"{Encoders.Hex.EncodeData(e.FingerPrint)}\"");
                    ...
                    else
                    {
                        ...
                        if (!e.CanTrust)
                            Logs.Configuration.LogError($"SSH host fingerprint for {e.HostKeyName} is untrusted, start GRSPay with -sshtrustedfingerprints \"{Encoders.Hex.EncodeData(e.FingerPrint)}\"");
                    }
                };
            }
            else
        ...
        public async Task<IActionResult> DeleteDynamicDnsService(string hostname)
        {
            ...
            return View("Confirm", new ConfirmModel()
            {
                ...
                Description = "GRSPayServer will stop updating this DNS record periodically",
                Action = "Delete"
            });
        }
        [HttpPost]
        ...
        public async Task<IActionResult> Emails(EmailsViewModel model, string command)
        {
            ...
            if (command == "Test")
            {
                try
                {
                    ...
                    var message = model.Settings.CreateMailMessage(new MailAddress(model.TestEmail), "GRSPay test", "GRSPay test");
