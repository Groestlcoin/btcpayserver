            ...
            else if (model.BumpMethod == "RBF")
            {
                ...
                IActionResult ChangeTooSmall(WalletBumpFeeViewModel vm, Money? missing)
                {
                    if (missing is not null)
                        ModelState.AddModelError(nameof(vm.FeeSatoshiPerByte), StringLocalizer["The change output is too small to pay for additional fee. (Missing {0} GRS)", missing.ToDecimal(MoneyUnit.BTC)]);
                    else
