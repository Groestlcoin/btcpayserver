...
namespace BTCPayServer.PluginPacker
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            await File.WriteAllTextAsync(outputFile + ".grspay.json", json);

            var sha256sums = new StringBuilder();
            sha256sums.AppendLine(
                $"{Encoders.Hex.EncodeData(Hashes.SHA256(Encoding.UTF8.GetBytes(json)))} {name}.grspay.json");

            sha256sums.AppendLine(
                $"{Encoders.Hex.EncodeData(Hashes.SHA256(await File.ReadAllBytesAsync(outputFile + ".grspay")))} {name}.grspay");
